...
import type { RemodelacionObraMock } from "@/data/mock/obras";

/** Selector compacto que presenta una sola comparación antes/después por vez. */
export function WorksRemodelations({
  remodelaciones,
}: {
  remodelaciones: RemodelacionObraMock[];
}) {
  const [remodelacionId, setRemodelacionId] = useState(
    remodelaciones[0]?.id ?? "",
  );
  const remodelacion =
    remodelaciones.find((item) => item.id === remodelacionId) ??
    remodelaciones[0];

  if (!remodelacion) return null;

  return (
    <Tabs
      value={remodelacion.id}
      onValueChange={(value) => {
        if (value) setRemodelacionId(String(value));
      }}
      className="gap-0"
    >
      {remodelaciones.length > 1 && (
          ...
          <figure>
            <div className="overflow-hidden rounded-xl bg-muted">
              <ProductImage
                ...
                alt={`Antes de la remodelación en ${remodelacion.ubicacion}`}
          ...
          <figure>
            <div className="overflow-hidden rounded-xl bg-muted">
              <ProductImage
                ...
                alt={`Después de la remodelación en ${remodelacion.ubicacion}`}
                sizes="(max-width: 768px) 100vw, 50vw"
                className="aspect-4/3 w-full"
                imgClassName="object-cover"
              />
